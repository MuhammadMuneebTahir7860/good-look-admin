...
export const getAllBlogs = (setGetLoading) => async (dispatch) => {
  try {
    setGetLoading(true);
    const response = await get(`${endPoint}api/getAllWorkerPost`);
    dispatch({
...
export const doAddBlog =
  ...
  async (dispatch) => {
    try {
      ...
      const res = await post(`${endPoint}api/addWorkerPost`, data);
      dispatch({
        type: ADD_WORKER_POST,
...
export const doDeleteBlog =
  (id, Toast, setSubmitLoading, setDelModal, user) => async (dispatch) => {
    try {
      setSubmitLoading(true);
      const res = await del(`${endPoint}api/deleteWorkerPost/${id}`);
      dispatch({
        type: DELETE_WORKER_POST,
        payload: id,
      });
      Toast(res?.data?.msg, "success", "colored");
    } catch (error) {
...
export const doUpdateBlog =
  ...
  async (dispatch) => {
    try {
      ...
      setSubmitLoading(true);
      const res = await put(`${endPoint}api/updateWorkerPost`, {
        data,
      });
      console.log(res, "res after update");
      dispatch({
        type: UPDATE_WORKER_POST,
        payload: data,
      });
      Toast(res?.data?.msg, "success", "colored");
      handleCloseUpdate();
